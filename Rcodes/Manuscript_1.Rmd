...
```{r CheckMeans}
# Check how many isoforms are detected on average  - STATS BEFORE MERGING THE TRANSCRIPT IDS
Avg_Transcript <- mean(c(dim(isoform_annotations_ccRCC_2_filtered_w_CellNumber)[1], dim(isoform_annotations_ccRCC_3_filtered_w_CellNumber)[1], dim(isoform_annotations_ccRCC_4_filtered_w_CellNumber)[1], dim(isoform_annotations_ccRCC_5_filtered_w_CellNumber)[1], dim(isoform_annotations_normal_filtered_w_CellNumber)[1])) # 86181.6

Avg_Novel <- mean(c(dim(isoform_annotations_ccRCC_2_filtered_w_CellNumber[isoform_annotations_ccRCC_2_filtered_w_CellNumber$structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog'),])[1], dim(isoform_annotations_ccRCC_3_filtered_w_CellNumber[isoform_annotations_ccRCC_3_filtered_w_CellNumber$structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog'),])[1], dim(isoform_annotations_ccRCC_4_filtered_w_CellNumber[isoform_annotations_ccRCC_4_filtered_w_CellNumber$structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog'),])[1], dim(isoform_annotations_ccRCC_5_filtered_w_CellNumber[isoform_annotations_ccRCC_5_filtered_w_CellNumber$structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog'),])[1], dim(isoform_annotations_normal_filtered_w_CellNumber[isoform_annotations_normal_filtered_w_CellNumber$structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog'),])[1])) # 31530.8

print(paste('Number of Average Transcript:', Avg_Transcript))
print(paste('Number of Average Novel Transcript:', Avg_Novel))
percentage <- Avg_Novel/Avg_Transcript*100
print(paste("Percentage of Novel Transcripts:", round(percentage, 2), "%"))
```
...
Statistical Test for the length of the isoforms
...
```
Proportion of structural categories across bins 
df_count %>%
...
df_count %>%
     ...
     summarize(total_count = sum(count), .groups = "drop") %>%
     group_by(bins) %>%
     mutate(percentage = (total_count / sum(total_count)) * 100) %>% dplyr::filter(bins == ">150")
...
df_all  %>% dplyr::group_by(Group) %>% dplyr::select(gene, Group, isoformcount) %>% dplyr::distinct() %>% dplyr::filter(isoformcount == 1)  %>% dplyr::summarise(totalNumber = n())
...
UMI_category_long_totalUMIperIsoform_plot <- ggplot(UMI_category_long_totalUMIperIsoform, aes(x = structural_category, y = totalUMI)) + 
  ...
  theme(legend.position="right") + 
  theme(axis.text.x = element_blank())
UMI_category_long_totalUMIperIsoform_plot
...
# Percentage of Novel Isoforms found in one cell
mean(as.vector((isoform_annotations %>% dplyr::select(Group,isoform,CellNumber, structural_category) %>% dplyr::group_by(Group) %>% dplyr::filter(CellNumber == 1) %>% dplyr::filter(structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog')) %>% dplyr::summarise(isoform1 = n()) %>% dplyr::select(isoform1)) / (isoform_annotations %>% dplyr::select(Group,isoform,CellNumber, structural_category) %>% dplyr::group_by(Group) %>% dplyr::filter(structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog')) %>% dplyr::summarise(isoform1 = n()) %>% dplyr::select(isoform1)))$isoform1)
63.3
...
ORF_normal <- ORF_normal[ORF_normal$ORFType %in% c("5prime_partial (+)", "complete (+)",      "3prime_partial (+)", "internal (+)"),]
...
ORF_ccRCC2 <- ORF_ccRCC2[ORF_ccRCC2$ORFType %in% c("5prime_partial (+)" , "complete (+)",       "3prime_partial (+)", "internal (+)"),]
...
ORF_ccRCC3 <- ORF_ccRCC3[ORF_ccRCC3$ORFType %in% c("5prime_partial (+)" , "complete (+)",       "3prime_partial (+)", "internal (+)"),]
...
ORF_ccRCC4 <- ORF_ccRCC4[ORF_ccRCC4$ORFType %in% c("5prime_partial (+)" , "complete (+)",       "3prime_partial (+)", "internal (+)"),]
...
ORF_ccRCC5 <- ORF_ccRCC5[ORF_ccRCC5$ORFType %in% c("5prime_partial (+)" , "complete (+)",       "3prime_partial (+)", "internal (+)"),]
```
...
## prop trend test - Cochran - Armitage test for trend 
successes <- contingency_table[,3]  # Counts of "complete (+)" ORFs
totals <- rowSums(contingency_table)  # Total counts of all ORF categories
result <- prop.trend.test(successes, totals)
print(result)
```
...
### Figure 2D
```{r Figure2D}
ORF_ccRCC2_seurat_iupred_FSM <- merge(iupred_result_all_ccRCC2, ORF_ccRCC2_seurat[ORF_ccRCC2_seurat$structural_category  == 'full-splice_match' & ORF_ccRCC2_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC2_seurat_iupred_ISM <- merge(iupred_result_all_ccRCC2, ORF_ccRCC2_seurat[ORF_ccRCC2_seurat$structural_category  == 'incomplete-splice_match' & ORF_ccRCC2_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC2_seurat_iupred_Novel <- merge(iupred_result_all_ccRCC2, ORF_ccRCC2_seurat[ORF_ccRCC2_seurat$structural_category  %in% c('novel_in_catalog', 'novel_not_in_catalog')  & ORF_ccRCC2_seurat$ORFType %in% c('complete (+)'),], by='isoform')

ORF_ccRCC3_seurat_iupred_FSM <- merge(iupred_result_all_ccRCC3, ORF_ccRCC3_seurat[ORF_ccRCC3_seurat$structural_category  == 'full-splice_match'  & ORF_ccRCC3_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC3_seurat_iupred_ISM <- merge(iupred_result_all_ccRCC3, ORF_ccRCC3_seurat[ORF_ccRCC3_seurat$structural_category  == 'incomplete-splice_match'& ORF_ccRCC3_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC3_seurat_iupred_Novel <- merge(iupred_result_all_ccRCC3, ORF_ccRCC3_seurat[ORF_ccRCC3_seurat$structural_category  %in% c('novel_in_catalog', 'novel_not_in_catalog') & ORF_ccRCC3_seurat$ORFType %in% c('complete (+)'),], by='isoform')

ORF_ccRCC4_seurat_iupred_FSM <- merge(iupred_result_all_ccRCC4, ORF_ccRCC4_seurat[ORF_ccRCC4_seurat$structural_category  == 'full-splice_match' & ORF_ccRCC4_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC4_seurat_iupred_ISM <- merge(iupred_result_all_ccRCC4, ORF_ccRCC4_seurat[ORF_ccRCC4_seurat$structural_category  == 'incomplete-splice_match' & ORF_ccRCC4_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC4_seurat_iupred_Novel <- merge(iupred_result_all_ccRCC4, ORF_ccRCC4_seurat[ORF_ccRCC4_seurat$structural_category  %in% c('novel_in_catalog', 'novel_not_in_catalog')  & ORF_ccRCC4_seurat$ORFType %in% c('complete (+)'),], by='isoform')

ORF_ccRCC5_seurat_iupred_FSM <- merge(iupred_result_all_ccRCC5, ORF_ccRCC5_seurat[ORF_ccRCC5_seurat$structural_category  == 'full-splice_match'  & ORF_ccRCC5_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC5_seurat_iupred_ISM <- merge(iupred_result_all_ccRCC5, ORF_ccRCC5_seurat[ORF_ccRCC5_seurat$structural_category  == 'incomplete-splice_match'  & ORF_ccRCC5_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_ccRCC5_seurat_iupred_Novel <- merge(iupred_result_all_ccRCC5, ORF_ccRCC5_seurat[ORF_ccRCC5_seurat$structural_category  %in% c('novel_in_catalog', 'novel_not_in_catalog')  & ORF_ccRCC5_seurat$ORFType %in% c('complete (+)'),], by='isoform')

ORF_seurat_iupred_FSM <- merge(iupred_result_all_Normal, ORF_N_seurat[ORF_N_seurat$structural_category  == 'full-splice_match'  & ORF_N_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_seurat_iupred_ISM <- merge(iupred_result_all_Normal, ORF_N_seurat[ORF_N_seurat$structural_category  == 'incomplete-splice_match' & ORF_N_seurat$ORFType %in% c('complete (+)'),], by='isoform')
ORF_seurat_iupred_Novel <- merge(iupred_result_all_Normal, ORF_N_seurat[ORF_N_seurat$structural_category  %in% c('novel_in_catalog', 'novel_not_in_catalog') & ORF_N_seurat$ORFType %in% c('complete (+)'),], by='isoform')
...
ORF_ccRCC2_seurat_iupred_FSM_forRM  <- merge(iupred_result_all_ccRCC2, ORF_ccRCC2_seurat_forRM[ORF_ccRCC2_seurat_forRM$structural_category == 'full-splice_match',] , by='isoform')
ORF_ccRCC3_seurat_iupred_FSM_forRM  <- merge(iupred_result_all_ccRCC3, ORF_ccRCC3_seurat_forRM[ORF_ccRCC3_seurat_forRM$structural_category == 'full-splice_match',], by='isoform')

ORF_ccRCC4_seurat_iupred_FSM_forRM  <- merge(iupred_result_all_ccRCC4, ORF_ccRCC4_seurat_forRM[ORF_ccRCC4_seurat_forRM$structural_category == 'full-splice_match',], by='isoform')
ORF_ccRCC5_seurat_iupred_FSM_forRM  <- merge(iupred_result_all_ccRCC5, ORF_ccRCC5_seurat_forRM[ORF_ccRCC5_seurat_forRM$structural_category == 'full-splice_match',], by='isoform')
ORF_seurat_iupred_FSM_forRM  <- merge(iupred_result_all_Normal, ORF_N_seurat_forRM[ORF_N_seurat_forRM$structural_category == 'full-splice_match',], by='isoform')
...
ORF_ccRCC2_seurat_iupred_Novel[ORF_ccRCC2_seurat_iupred_Novel$isoform %in% isoform_annotations_ccRCC_2_filtered_w_CellNumber[isoform_annotations_ccRCC_2_filtered_w_CellNumber$isoform %in% ORF_ccRCC2[ORF_ccRCC2$ORFType %in% c('complete (+)', 'complete (-)'), 'PB.id'] & isoform_annotations_ccRCC_2_filtered_w_CellNumber$associated_gene.x == 'NNMT' & isoform_annotations_ccRCC_2_filtered_w_CellNumber$structural_category %in% c('novel_in_catalog', 'novel_not_in_catalog'), 'isoform'],]

## UMI counts across Disordered and Ordered Categories

## Selecting an Example Case
```{r Disordered_Ordered}
max_average <- ORF_iupred_subNNIC_NIC %>%
    dplyr::select(associated_gene.x, subcategory, structural_category, isoform, Disordered, Ordered) %>%
    dplyr::group_by(associated_gene.x, subcategory, structural_category) %>%
    dplyr::summarise(averageOrdered = mean(Ordered), .groups = 'drop') %>%
    dplyr::group_by(associated_gene.x) %>%
    dplyr::filter(averageOrdered == min(averageOrdered)) %>%
    dplyr::ungroup()
